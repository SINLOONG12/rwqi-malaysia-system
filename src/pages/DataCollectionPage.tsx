...
const DataCollectionPage: React.FC = () => {
  ...
  const [selectedCameraId, setSelectedCameraId] = useState<string | null>(searchParams.get('camera-id'));
  useEffect(() => {
  ...
  return <div className="container max-w-7xl mx-auto py-8 px-4 sm:px-6">
      <div className="mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
          ...
          <div className="mt-4 md:mt-0 flex items-center gap-3">
            <NotificationCenter userRole={userRole} onNotificationClick={handleNotificationClick} />
      ...
      <div className="bg-card rounded-xl shadow-sm border p-1">
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
          <TabsList className="grid grid-cols-6 mb-6 bg-muted/50 p-1 rounded-lg mx-px">
            ...
            {userRole === "publisher" && <TabsTrigger value="publisher-dashboard" className="rounded-md data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm">
                Publisher Dashboard
              </TabsTrigger>}
          </TabsList>
          ...
          <div className="p-4">
            <TabsContent value="camera-feeds">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {selectedCameraId && <div className="lg:col-span-1">
                    <LiveCameraFeedAnalysis cameraId={selectedCameraId} location={selectedCameraId === "cam-001" ? "Sungai Klang" : selectedCameraId === "cam-002" ? "Sungai Pinang" : "Sungai Langat"} flowData={[]} />
                    <div className="mt-4">
                      <button className="w-full px-4 py-2 text-sm text-muted-foreground border rounded-md hover:bg-muted" onClick={() => setSelectedCameraId(null)}>
                        Close Analysis
                      </button>
                    </div>
                  </div>}
            ...
            {userRole === "publisher" && <TabsContent value="publisher-dashboard">
                <PublisherDashboard />
              </TabsContent>}
          </div>
        </Tabs>
